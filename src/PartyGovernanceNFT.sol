...
abstract contract PartyGovernanceNFT is PartyGovernance, ERC721, IERC2981 {
    ...
    event PartyCardIntrinsicVotingPowerSet(
    ...
    constructor(
        IGlobals globals
    ) payable PartyGovernance(globals) ERC721("", "") {
        _GLOBALS = globals;
    }
    ...
    function _initialize(
        ...
        if (rageQuitTimestamp_ != 0) {
            if (
                proposalEngineOpts.distributionsConfig ==
    ...
    function getVotingPowerShareOf(
        uint256 tokenId
    ) public view returns (uint256) {
        uint256 totalVotingPower = _getSharedProposalStorage()
            .governanceValues
            .totalVotingPower;
        return
            totalVotingPower == 0
                ? 0
                : (votingPowerByTokenId[tokenId] * 1e18) / totalVotingPower;
    }
    ...
    function increaseVotingPower(uint256 tokenId, uint96 votingPower) external {
        ...
        if (
            totalVotingPower != 0 &&
            totalVotingPower - mintedVotingPower_ < votingPower
        ) {
            unchecked {
        ...
        uint256 newIntrinsicVotingPower = votingPowerByTokenId[tokenId] +
            votingPower;
        ...
        _adjustVotingPower(
            ownerOf(tokenId),
            votingPower.safeCastUint96ToInt192(),
            address(0)
        );
    ...
    function _burnAndUpdateVotingPower(
        ...
        for (uint256 i; i < tokenIds.length; ++i) {
            ...
            uint96 votingPower = votingPowerByTokenId[tokenId]
    ...
    function rageQuit(
        ...
        uint256[] memory withdrawAmounts = new uint256[](withdrawTokens.length);
        {
            ...
            for (uint256 i; i < withdrawTokens.length; ++i) {
                ...
                if (prevToken >= withdrawTokens[i])
                    revert InvalidTokenOrderError();
        ...
        }
        {
            ...
            uint96 totalVotingPowerBurned = _burnAndUpdateVotingPower(
                tokenIds,
                !isAuthority_
            );
            ...
            _getSharedProposalStorage()
                .governanceValues
                .totalVotingPower -= totalVotingPowerBurned;
        }
        {
